...
const Dashboard = () => {
 ...
 const handleSubmit = async () => {
  ...
  try {
    ...
    // Log full response for debugging when server shapes vary
    console.log("createWorkspace response:", response);

    const data = response?.data || {};

    if (!data) {
      alert("Unexpected server response when creating workspace");
      return;
    }
    ...
    const workspaceToken = data.workspaceToken || data.token || data.data?.workspaceToken || data.data?.token || null;

    if (!workspaceId) {
      console.error("No workspaceId found in createWorkspace response", data);
      alert("Workspace created but response did not include workspace id. Check console for response shape.");
      return;
    }

    // Store values we could extract
    try {
      localStorage.setItem("workspaceId", workspaceId); // active workspace
if (inviteCode) localStorage.setItem("inviteCode", inviteCode);
if (workspaceToken) {
  localStorage.setItem(`workspaceToken_${workspaceId}`, workspaceToken); // scoped token
}

    } catch (e) {
      console.warn("Failed to write workspace info to localStorage", e);
    }
