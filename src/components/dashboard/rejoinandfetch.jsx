...
const [modalOpen, setModalOpen] = useState(false);
    ...
    const fetchWorkspaces = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setToast({ show: true, type: "error", message: "Failed to fetch workspaces" });
        }
      } catch (error) {
    ...
    if (count === 3) {
      ...
      if (confirmed) {
        deleteWorkspace(workspaceId)
          .then(() => {
            setToast({ show: true, type: "success", message: "Workspace deleted successfully." });
            setWorkspaces((prev) => prev.filter((ws) => ws._id !== workspaceId));
          })
          .catch((err) => {
            setToast({ show: true, type: "error", message: err.message || "Failed to delete workspace." });
          });
      }
  ...
  const handleRejoin = async (wsOrId) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setToast({ show: true, type: "error", message: response.data?.message || "Failed to rejoin workspace" });
      }
    } catch (error) {
      console.error("Rejoin error:", error);
      setToast({ show: true, type: "error", message: error.response?.data?.message || error.message || "Something went wrong" });
    } finally {
      // If we navigate away, this doesn't matter; if not, we re-enable the button
      setTimeout(() => setRejoiningId((id) => (id === workspaceId ? null : id)), 1100);
    }
  };
  ...
  const handleCopyInvite = async (code, e) => {
    ...
    const value = code || "";
    if (!value) {
      setToast({ show: true, type: "error", message: "No invite code to copy" });
      return;
    }
    try {
      ...
      <div className="flex gap-12">
        {workspaces.map((ws) => (
          <div
            ...
            className="w-[324px] min-w-[324px] bg-[#EFE7F6] border border-[#8F7AA9] rounded-2xl p-6 shadow-sm flex flex-col items-center justify-between cursor-pointer"
          >
            <h3 className="text-2xl font-bold text-[#0E1219] mb-3 text-center">{ws.workspaceName}</h3>
            <div className="flex items-center gap-2 mb-6">
              <span className="text-sm text-[#000 ]">Invite code:</span>
              <code className="text-sm font-semibold tracking-wider text-[#0E1219] bg-[#dcd3e4] px-2 py-1 rounded">
                {ws.inviteCode || (ws._id === localStorage.getItem("workspaceId") ? localStorage.getItem("inviteCode") : "—")}
              </code>
              <button
                onClick={(e) => handleCopyInvite(ws.inviteCode || localStorage.getItem("inviteCode"), e)}
            ...
            >
              {rejoiningId === ws._id ? "Rejoining…" : "Rejoin"}
            </button>
          </div>
        ))}
      </div>
